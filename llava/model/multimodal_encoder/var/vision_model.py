...
class ResidualAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        # assert q_x.shape[0] == k_x.shape[0] == v_x.shape[0] == attn_mask.shape[0], "Batch sizes do not match"
        # assert q_x.shape[1] == attn_mask.shape[1], "Query and attention mask sequence lengths do not match"
        # assert k_x.shape[1] == v_x.shape[1] == attn_mask.shape[
        #     2], "Key, value, and attention mask key lengths do not match"
...
class PromptedVisionTransformer(nn.Module, PyTorchModelHubMixin):
    ...
    def forward(self, x: torch.Tensor, prompt: torch.Tensor, attn_mask: torch.Tensor):
        ...
        ## VaR Cross attention
        print(prompt.shape)
        print(self.text_embedding(prompt).shape)
        print(self.text_projector(self.text_embedding(prompt)).shape)
        ...
        x = self.cross_attention(q_x=x, k_x=prompt, v_x=prompt, attn_mask=attn_mask)
